  ...
  const statusCodeCheck = (code) => {
    if (code !== null) {
      if (code === 401) {
        ...
        return (
          <p style={{ color: "red" }}>Sorry, an error occured! Try again. </p>
        );
      }
    } else {
      return (
        <p>
          Log in to your Producktiv account.
          <br />
          Test without Logging in or Signing up?{" "}
          <button className="test-run" onClick={testRunLogin}>
            Click here
          </button>
        </p>
      );
