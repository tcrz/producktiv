...
export const Usercourses = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(
      `https://producktiv-backend.onrender.com/api/users/videos?${document.cookie}`,
      { credentials: "include" }
    )
      .then((response) => {
        if (!response.ok) {
  ...
  const deleteVideo = (id) => {
    ...
    fetch(
      `https://producktiv-backend.onrender.com/api/videos/${id}?${document.cookie}`,
      {
        method: "DELETE",
        credentials: "include",
      }
    ...
    <>
      <div className="all-courses user-courses">
        ...
        {!isLoading && (
          <>
            {videos.length === 0 ? (
              <p>Nothing to see here, you have not submitted any courses.</p>
            ) : (
              <>
